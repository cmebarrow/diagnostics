...
/**
 * This class is a subclass of log4j RollingFileAppender. It stores all logged messages in memory until (and if) there is a match of triggerPattern in the message, then static method printAllMessages
...
public class TriggeredRollingFileAppender extends RollingFileAppender {
    ...
    private static AtomicReference<BlockingQueue<LoggingEvent>> eventsList = 
    		new AtomicReference<BlockingQueue<LoggingEvent>> ();
    static {
    ...
    private static ExecutorService executorService = Executors.newSingleThreadExecutor();  // create a thread for printAllMessages 
    ...
    protected void subAppend(LoggingEvent event) {
        boolean match = false;
        String oldMessage = (String) event.getMessage();
        ...
        if (printNow) {
	        ...
	        if (match) {
		        ...
		        executorService.submit(new Runnable() {
                    public void run() {
		                debug(getDateTime () + "TriggeredRollingFileAppender instance execute printAllMessages Asynchronously");
    ...
    private String getDateTime() {
	 ...
	 try {
         ...
         String newMessage = oldMessage.substring(0,truncateAfter);
    ...
    private void appendAll(Queue<LoggingEvent> EL) {
        ...
        try {
            for (LoggingEvent event : eventsListOut) {
                super.append(event);
            }
        }
        finally {
